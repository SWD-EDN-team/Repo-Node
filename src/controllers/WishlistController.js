...
export const getWishlistBytoken = async (req, res) => {
  try {
    const wishlists = await Wishlist.find({user_id:req.user.id}).populate({
      path: 'product_id',
      populate: { path: 'category_id' }
    }).populate("user_id");
    if (!wishlists) res.status(StatusCode.NOT_FOUND).json({ message: "Wishlist not found" });
    res.status(StatusCode.OK).json({ message: "Get all wishlists in cart with token" ,wishlists});
  } catch (error) {
